...
struct CNodeState {
    ...
    //! Accumulated misbehaviour score for this peer which offer ipfs storage service.
    int nIpfsMisbehavior;
    //! Whether this peer should be disconnected and banned (unless whitelisted).
    bool fShouldBan;
...
static void PushNodeVersion(CNode* pnode, CConnman* connman, int64_t nTime)
{
    ...
    connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, 
        PROTOCOL_VERSION, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe, nonce, strSubVersion, nNodeStartingHeight, ::g_relay_txes));
...
/**
 * Prevent to upload blocks to misbehaving IPFS peer
 * Humphrey
 * 20211205
 */
void IPFSMisbehaving(NodeId pnode, int howmuch, const std::string& message) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    if (howmuch == 0)
        return;
    ...
    state->nIpfsMisbehavior -= howmuch;
    int banscore = gArgs.GetArg("-ipfsbanscore", DEFAULT_IPFS_BANSCORE_THRESHOLD);
    std::string message_prefixed = message.empty() ? "" : (": " + message);
    if (state->nIpfsMisbehavior <= banscore && state->nIpfsMisbehavior - howmuch < banscore) {
        LogPrint(BCLog::NET, "%s: %s peer=%d (%d -> %d) REPUTATION THRESHOLD EXCEEDED%s\n", __func__, state->name, pnode,
            state->nIpfsMisbehavior - howmuch, state->nIpfsMisbehavior, message_prefixed);
        state->fShouldBan = true;
    } else {
        LogPrint(BCLog::NET, "%s: %s peer=%d (%d -> %d)%s\n", __func__, state->name, pnode, state->nIpfsMisbehavior - howmuch,
            state->nIpfsMisbehavior, message_prefixed);
    }
}
